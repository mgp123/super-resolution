...
model_path = "saved_weights/trainning_frames.model"
spatial_size = 64
batch_size = 1
...
data_loader_train, data_loader_test = get_data_loaders(4, 2, slice_size=1024, random_crop=True,dataset_path="local/frames")
...
# sample_image = torchvision.io.read_image(path)/255
low_f = mean_and_blurr(kernel_size=5)
...
for spatial_dim, low_dim in [(128, 64)]:
    ...
    sample_image = low_f(hr)


    # sample_image.unsqueeze_(0)
    sample_image = sample_image.to("cuda:0")
    sr = make_superres(g,sample_image, overlap=0,kernel_size=128)
    ...
    torchvision.utils.save_image(image_grid,f"sr3_{spatial_dim}.png")
