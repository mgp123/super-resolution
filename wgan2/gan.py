...
class WGAN(pl.LightningModule):
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor]):
        ...
        for _ in range(self.training_parameters["discriminator_iterations_per_batch"]):
            ...
            lipschitz_loss = torch.linalg.norm(
                gradients_interpolation - 1.0, dim=1
            ).mean()
            discrminator_loss = (
                score_loss
                + lipschitz_loss
                * self.training_parameters["coefficient_lipschitz_loss"]
            )
    ...
    def configure_optimizers(self):
        optimizer_g = torch.optim.Adam(
            ...
            lr=self.training_parameters["lr_g"],
            betas=self.training_parameters["betas"],
        )
        optimizer_d = torch.optim.Adam(
            self.discriminator.parameters(),
            lr=self.training_parameters["lr_d"],
            betas=self.training_parameters["betas"],
        )
    ...
    def gradient_penalty(self, module, hr, sr):
        ...
        random_interpolation = torch.autograd.Variable(
        ...
        gradients_interpolation = gradients_interpolation.view(
            (gradients_interpolation.shape[0], -1)
        )
